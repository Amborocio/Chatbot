...
def show_best_accommodation(user_id, city_name):
    ...
    accommodations_to_show = sorted_data.iloc[start_index:start_index + 1]
...
def show_famous_food(user_id, city_name):
    ...
    if user_id not in pagination_state:
        pagination_state[user_id] = {'foods': 0}
...
def show_food_locations(user_id, city_name, query):
    ...
    food_name = query.split("where can i buy")[1].split("in")[0].strip()
...
def show_food_type(user_id, city_name, query):
    ...
    food_name = query.split("what type of food is")[1].split("in")[0].strip()
...
def query():
    ...
    if user_query and user_id:
        ...
        if "no" in user_query_lower:
            ...
            pagination_state[user_id]['city_name'] = None
